...
type GpsCoordinate struct {
	...
	AccuracyMeters decimal.Decimal `json:"accuracyMeters"`
}

type PurchaseProduct struct {
	Quantity         string          `json:"quantity"`
	VatPercentage    decimal.Decimal `json:"vatPercentage"`
...
func (c *Client) Purchases() (*Purchases, error) {
	...
	return &Purchases{Purchases: resp.Purchases}, nil
}
	...
	for _, purchase := range s.purchases.Purchases {
		for i := range purchase.Products {
			...
			v.Purchase = append(v.Purchase, PurchaseSummary{
				Product: &product,
				Count:   count,
				Amount:  product.UnitPrice * count,
			})
...
func (p Purchases) GroupByDate() map[string]Purchases {
	...
	for _, dp := range p.Purchases {
		date := strings.Split(dp.Timestamp, "T")[0]
